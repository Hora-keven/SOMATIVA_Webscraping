...
class Tela:
    ...
    def procura(self) -> None:
        ...
        for i in banco.cursor.execute(f"SELECT * FROM celulares WHERE marca LIKE '%{self.clicked.get().lower()}%'"):
            self.listaCli.insert(parent="", index=0, values=i)
    ...
    def opcao_csv(self):
        ...
        for i in banco.cursor.execute(f"SELECT * FROM celulares WHERE marca LIKE '%{self.clicked.get().lower()}%'"):
            ...
            lista_modelo.append(i[1])
            lista_valor.append(i[2])
            print(lista_marca)
            print(lista_valor)
            print(lista_modelo)
            ...
            data.to_csv(f'C:/Users/53688621808/Documents/planilhas/{self.clicked.get()}.csv', sep=";", index=False)
